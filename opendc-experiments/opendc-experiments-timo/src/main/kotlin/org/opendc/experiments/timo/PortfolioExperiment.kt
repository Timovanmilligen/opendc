...
import org.opendc.compute.workload.telemetry.CompositeComputeMonitor
...
class PortfolioExperiment {
    ...
    private fun runGeneticSearch(folderName: String, range: IntRange) {
        ...
        val engine = Engine.builder(SnapshotProblem(snapshots, createTopology(topologyName)))
        ...
        geneticSearchWriter.write(
            "${result.generation()} ${result.population().map{it.fitness()}.average()} " +
                "${result.bestFitness()} ${FilterScheduler(
                    weighers = schedulerSpec.weighers, filters = schedulerSpec.filters,
                    subsetSize = schedulerSpec.subsetSize
                )} ${schedulerSpec.vCpuOverCommit}"
        )
        geneticSearchWriter.flush()
     ...
     * Obtain the trace.
     */
    private fun createWorkload(traceName: String, seed: Int = 0): ComputeWorkload.Resolved {
        return trace(traceName).resolve(workloadLoader, Random(seed.toLong()))
    }

    private fun createPortfolio(): Portfolio {
        val portfolio = Portfolio()
        val lowestCpuDemand = PortfolioEntry(
            FilterScheduler(
                filters = listOf(ComputeFilter(), VCpuFilter(vCpuAllocationRatio), RamFilter(ramAllocationRatio)),
                weighers = listOf(CpuDemandWeigher())
            ),
        ...
        val lowestCpuLoad = PortfolioEntry(
            FilterScheduler(
                filters = listOf(ComputeFilter(), VCpuFilter(vCpuAllocationRatio), RamFilter(ramAllocationRatio)),
                weighers = listOf(CpuLoadWeigher())
            ),
        ...
        val vCpuCapacityWeigher = PortfolioEntry(
            FilterScheduler(
                filters = listOf(ComputeFilter(), VCpuFilter(vCpuAllocationRatio), RamFilter(ramAllocationRatio)),
                weighers = listOf(VCpuCapacityWeigher())
            ),
        ...
        val lowestMemoryLoad = PortfolioEntry(
            FilterScheduler(
                filters = listOf(ComputeFilter(), VCpuFilter(vCpuAllocationRatio), RamFilter(ramAllocationRatio)),
                weighers = listOf(RamWeigher())
            ),
        ...
        val maximumConsolidationLoad = PortfolioEntry(
            FilterScheduler(
                filters = listOf(ComputeFilter(), VCpuFilter(vCpuAllocationRatio), RamFilter(ramAllocationRatio)),
                weighers = listOf(MCLWeigher())
            ),
        ...
        val geneticSearchResult = PortfolioEntry(
            FilterScheduler(
                filters = listOf(ComputeFilter(), VCpuFilter(24.0), RamFilter(1.0)),
                weighers = listOf(InstanceCountWeigher(0.9412430903700983)), subsetSize = 9
            ),
            Long.MAX_VALUE, 0
        )
        val secondGeneticSearchResult = PortfolioEntry(
            FilterScheduler(
        ...
        val bitbrainsGeneticResult = PortfolioEntry(
            FilterScheduler(
                filters = listOf(ComputeFilter(), VCpuFilter(20.0), RamFilter(1.0)),
                weighers = listOf(CoreRamWeigher(-0.6865062188603075)), subsetSize = 13
            ),
            Long.MAX_VALUE, 0
        )
        val bitbrainsGeneticResult2 = PortfolioEntry(
            FilterScheduler(
                filters = listOf(ComputeFilter(), VCpuFilter(48.0), RamFilter(1.0)),
                weighers = listOf(CoreRamWeigher(-0.9087571514776227), VCpuWeigher(allocationRatio = 48.0, multiplier = 0.511676672730697)), subsetSize = 4
            ),
            Long.MAX_VALUE, 0
        )
